...
def main(cfg: DictConfig):
    ...
    wandb.init(project='MLE-mujoco', settings=wandb.Settings(_disable_stats=True), \
        group=args.env.name,
        job_type=f'MLE-{args.env.demo.split("-")[0].split("_")[-1]}({args.expert.demos})'+
        f'-{args.env.sub_optimal_demo.split("-")[0].split("_")[-1]}({args.env.num_sub_optimal_demo})',
    ...
    print(args.env.num_sub_optimal_demo)
    add_memory_replay = Memory(1, args.seed)
    add_memory_replay.load(hydra.utils.to_absolute_path(f'experts/{args.env.sub_optimal_demo}'),
                              num_trajs=args.env.num_sub_optimal_demo,
                              sample_freq=1,
                              seed=args.seed + 432)
    print(f'--> Add memory size: {add_memory_replay.size()}')
...
def iq_update_critic(self, policy_batch, add_batch, expert_batch,step):
    args = self.args
    batch = get_concat_with_add_samples(policy_batch, add_batch, expert_batch, args)
...
def iq_update(self, policy_buffer, add_buffer, expert_buffer, step):
    policy_batch = policy_buffer.get_samples(self.batch_size, self.device)
    add_batch = add_buffer.get_samples(self.batch_size, self.device)
    expert_batch = expert_buffer.get_samples(self.batch_size, self.device)

    losses = self.iq_update_critic(policy_batch ,add_batch, expert_batch,step)
    if self.actor and step % self.actor_update_frequency == 0:
        if self.args.offline:
            ...
            obs = torch.cat([policy_batch[0], add_batch[0], expert_batch[0]], dim=0)
