...
# env.name=HalfCheetah-v3 method.loss=neg_value \
...
# env.name=Ant-v3 method.loss=neg_value \
...
CUDA_VISIBLE_DEVICES=3 python train_both.py env=humanoid agent=sac \
...
env.num_sub_optimal_demo=[100000,50000,20000,10000,1000] \
expert.reward_arr=[0.3,0.6,0.9,1.2,1.5] \
agent.actor_lr=3e-5 seed=0

# CUDA_VISIBLE_DEVICES=3 python train_both.py env=walker2d agent=sac \
# env.name=Walker2d-v3 method.loss=neg_value \
# agent.init_temp=0.03 env.eval_interval=1e4 \
...
# env.name=Hopper-v3 method.loss=neg_value \
# agent.init_temp=0.03 env.eval_interval=1e4 \
